<script lang="ts">
	...
	async function searchRides() {
		...
		if (error) {
			...
			results = ridesWithDrivers.filter((ride) => {
				...
				const matchesDeparture = (query: string) =>
					rideDeparture.includes(query) || ridePickup.includes(query);
				const matchesArrival = (query: string) =>
					rideArrival.includes(query) || rideDropoff.includes(query);

				let routeMatches = true;
				if (depLower && arrLower) {
					routeMatches = matchesDeparture(depLower) && matchesArrival(arrLower);
				} else if (depLower) {
					routeMatches = matchesDeparture(depLower) || matchesArrival(depLower);
				} else if (arrLower) {
					routeMatches = matchesDeparture(arrLower) || matchesArrival(arrLower);
				}
				...
				const girlsOnlyMatch = ride.girls_only
					? canAccessGirlsOnlyRides(currentMemberStatus, isFemaleUser ? 'female' : 'male')
					: true;
	...
	onMount(async () => {
		...
		if (user) {
			await supabase.rpc('refresh_review_pending_for_member', { p_user_id: user.id });
						...
						<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
							<div>
								<div class="flex items-center gap-3 mb-2">
									...
									{:else}
										<div class="relative inline-flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-200 ring-2 ring-white">
											{#if ride.driver_profile?.profile_photo_url}
												<img
													src={ride.driver_profile.profile_photo_url}
													alt={driverDisplayName(ride)}
									...
									<div class="flex items-center gap-2 min-w-0">
										...
										{:else}
											<span class="text-sm font-semibold text-slate-800 truncate">{driverDisplayName(ride)}</span>
										{/if}
