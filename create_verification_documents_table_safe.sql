-- Safe, complete migration for verification_documents
-- Run this full script in one shot (not partial selection)
...
-- Only enforce NOT NULL if data is now present
DO $$
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (SELECT 1 FROM public.verification_documents WHERE user_id IS NULL) THEN
      EXECUTE 'ALTER TABLE public.verification_documents ALTER COLUMN user_id SET NOT NULL';
    END IF;
  END IF;
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (SELECT 1 FROM public.verification_documents WHERE document_type IS NULL) THEN
      EXECUTE 'ALTER TABLE public.verification_documents ALTER COLUMN document_type SET NOT NULL';
    END IF;
  END IF;
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (SELECT 1 FROM public.verification_documents WHERE file_name IS NULL) THEN
      EXECUTE 'ALTER TABLE public.verification_documents ALTER COLUMN file_name SET NOT NULL';
    END IF;
  END IF;
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (SELECT 1 FROM public.verification_documents WHERE storage_path IS NULL) THEN
      EXECUTE 'ALTER TABLE public.verification_documents ALTER COLUMN storage_path SET NOT NULL';
    END IF;
  END IF;
END $$;

CREATE UNIQUE INDEX IF NOT EXISTS verification_documents_storage_path_unique_idx
...
FOR EACH ROW
EXECUTE FUNCTION public.set_verification_documents_updated_at();
